...
const sendEmail = async (to, subject, html) => {
  try {
    ...
    const info = await transporter.sendMail({
      from: `"Smartlover" <${process.env.EMAIL_FROM}>`,
      to,
      subject,
      html,
    });
...
const orderConfirmationEmail = (order, customerName) => {
  ...
  return {
    subject: `Smartlover — Order Confirmed #${order._id.toString().slice(-8).toUpperCase()}`,
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8faf8;">
        <div style="background: linear-gradient(135deg, #059669, #10b981); padding: 30px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 28px;">💄 Smartlover</h1>
        ...
        <div style="background: #f1f5f9; padding: 20px; text-align: center;">
          <p style="color: #94a3b8; font-size: 12px; margin: 0;">© ${new Date().getFullYear()} Smartlover. Premium fashion and beauty delivered with care.</p>
        </div>
      </div>
    `,
  };
...
const deliveryAssignmentEmail = (order, deliveryGuyName) => ({
  subject: `Smartlover — New Delivery Assignment #${order._id.toString().slice(-8).toUpperCase()}`,
  html: `
...
const salaryPaidEmail = (employeeName, payroll) => ({
  subject: `Smartlover — Salary Credited for ${payroll.month}/${payroll.year}`,
  html: `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: linear-gradient(135deg, #059669, #10b981); padding: 30px; text-align: center;">
...
const welcomeEmail = (name) => ({
  subject: 'Welcome to Smartlover! ✨',
  html: `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: linear-gradient(135deg, #059669, #10b981); padding: 40px; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 32px;">💄 Smartlover</h1>
        <p style="color: #d1fae5; margin: 10px 0 0; font-size: 16px;">Premium fashion and beauty, delivered with care</p>
      </div>
      <div style="padding: 30px; background: white;">
        <h2 style="color: #1e293b; margin-top: 0;">Welcome, ${name}! 🎉</h2>
        <p style="color: #64748b; line-height: 1.6;">Thank you for joining Smartlover! Explore our curated range of fashion, cosmetics, skincare, and beauty essentials.</p>
...
const paymentReceiptEmail = (order, customerName) => {
  ...
  return {
    subject: `Smartlover — Payment Receipt #${orderId}`,
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #059669, #0d9488); padding: 35px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 28px; letter-spacing: -0.5px;">💄 Smartlover</h1>
        ...
        <!-- Footer -->
        <div style="background: #f1f5f9; padding: 20px; text-align: center;">
          <p style="color: #64748b; font-size: 13px; margin: 0 0 5px;">Thank you for shopping with Smartlover! ✨</p>
          <p style="color: #94a3b8; font-size: 11px; margin: 0;">© ${new Date().getFullYear()} Smartlover. Premium fashion and beauty delivered with care.</p>
        </div>
      </div>
    `,
  };
  ...
  return {
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto;">
        <h2 style="color:#059669;">Smartlover POS Receipt</h2>
...
const customerReturnUpdateEmail = ({ order, returnDoc }) => {
  ...
  return {
    subject: `Smartlover — Return Update ${rma}`,
    html: `
